...
from factor_backtest.io import ensure_dir, read_json, write_json
...
def _pending_diagnostic_payload(filename: str) -> dict:
    ...
    return {
        ...
        "reason": f"当前 Factor_Backtest 第一轮 handoff 尚未产出 {name} 诊断的 computed 数值。",
        "owner": "回测组/风控组/IT组",
...
def _handoff_readme() -> str:
    return """# Factor_Backtest Platform Handoff

本目录是公司 Factor_Backtest 给 Alpha-Lab/平台侧的第一轮最小闭环交付包。

`sample_latest/` 来自一次真实 Factor_Backtest 运行，目录内只保留验收所需的核心 JSON、CSV 和 diagnostics 占位文件。四类 diagnostics 第一轮允许为 `pending(company)`，不会伪造 computed 指标。
...
Factor_Backtest 的标准日频因子输入是宽表：index 为交易日期，columns 为股票代码，每个单元格是一只股票在一个交易日的因子值。日期会按 pandas datetime 处理，股票代码需要与行情、股票池、风险暴露数据的代码格式一致。CSV、parquet、HDF 或已经读取好的 DataFrame 均可接入，最终进入回测核心时都会对齐成该宽表结构。
